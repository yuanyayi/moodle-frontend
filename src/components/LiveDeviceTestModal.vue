          ...
          <a-card title="摄像头设置" size="small">
            ...
            <div class="camera-preview">
              <video 
                ...
                style="width: 100%; height: 150px; background-color: #000; border-radius: 4px;"
              ></video>
            </div>
          </a-card>
        </a-col>
        ...
        <a-col :span="12">
          <a-card title="设备状态" size="small">
            <div class="device-status">
              ...
              />
              <a-alert 
                v-else-if="microphoneStatus === 'denied'" 
                type="error" 
                message="麦克风权限被拒绝" 
                show-icon
                style="margin-top: 10px;"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const updateDeviceReadyStatus = () => {
      isDeviceReady.value = cameraStatus.value === 'granted' && 
                           microphoneStatus.value === 'granted' &&
                           cameraDevices.value.length > 0 &&
                           microphoneDevices.value.length > 0;
    };
