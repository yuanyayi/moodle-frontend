<template>
  <div>
    <!-- 子项合并的FormItem -->
    <a-form-item v-if="fieldDesc.type === 'children'" :name="field" :label="fieldDesc.label" v-bind="{ ...formItemLayout, ...fieldDesc.itemProps }">
      <FormItemWithChildren :key="field" v-bind="{ fieldDesc, field }" v-decorator="[field, fieldDesc.options]" @change="handleChange" />
    </a-form-item>
    <!-- 自定义的FormItem -->
    ...
    <!-- others -->
    <template v-else>
      <a-form-item v-if="fieldDesc.invisible !== true" v-show="fieldDesc.type !== 'hidden'" :name="field" :label="fieldDesc.label" v-bind="{ ...formItemLayout, ...fieldDesc.itemProps }">
        <!-- Text (default) -->
        <a-input v-if="!fieldDesc.type || fieldDesc.type === 'text' || fieldDesc.type === true" v-decorator="[field, fieldDesc.options]" v-bind="fieldDesc.props">
          <slot name="addonBefore" slot="addonBefore"></slot>
          <slot name="addonAfter" slot="addonAfter"></slot>
        </a-input>
        <!-- Hidden -->
        <a-input v-if="fieldDesc.type === 'hidden'" v-decorator="[field, { rules: [{ type: 'any' }] }]" v-bind="fieldDesc.props" />
        <!-- Number -->
        <template v-if="fieldDesc.type === 'number' || fieldDesc.type === 'price'">
          <a-input-group compact>
            <a-input-number
              ...
              @change="handleChange" />
            <span v-if="fieldDesc.props && fieldDesc.props.ps" style="width: 60%; padding: 5px 7px" :style="fieldDesc.props.ps.style">{{ fieldDesc.props.ps.text }}</span>
          </a-input-group>
        </template>
        <!-- textarea -->
        ...
        <a-select
          ...
          :placeholder="fieldDesc.props && fieldDesc.props.placeholder ? fieldDesc.props.placeholder : fieldDesc.inputSearch ? '输入文字进行搜索' : '请选择'"
          :optionFilterProp="(fieldDesc.props && fieldDesc.props.optionFilterProp) || (fieldDesc.inputSearch && 'children')"
        ...
        <a-tree-select
          ...
          :placeholder="fieldDesc.props && fieldDesc.props.placeholder ? fieldDesc.props.placeholder : fieldDesc.inputSearch ? '输入文字进行搜索' : '请选择城市'"
          :optionFilterProp="'children'"
          v-bind="fieldDesc.props"
          v-on="fieldDesc.actions"></a-tree-select>
        <!-- checkbox -->
        <template v-if="fieldDesc.type === 'checkbox'">
          <a-checkbox-group v-decorator="[field, fieldDesc.options]" v-bind="fieldDesc.props" @change="handleChange" style="width: 100%">
            <a-row>
          ...
          <a-checkbox-group v-decorator="[field, fieldDesc.options]" v-bind="fieldDesc.props" @change="handleChange" style="width: 100%">
            <a-row v-for="(list, key, index) in fieldDesc.list" :key="index">
              ...
              <a-col :span="8" v-for="item in list" :key="item.value">
                <a-checkbox :value="item.value || item[fieldDesc.itemValueName]" :disabled="item.disabled">{{ item.label || item[fieldDesc.itemLabelName] }}</a-checkbox>
              </a-col>
            </a-row>
          </a-checkbox-group>
        </template>
        ...
        <template v-if="fieldDesc.type === 'upload'">
          <a-upload :fileList="fileList" :showUploadList="true" @preview="uploadhandlePreview" @change="uploadhandleChange" v-decorator="[field, fieldDesc.options]" v-bind="fieldDesc.props">
        ...
        <template v-if="fieldDesc.type === 'switch'">
          <a-switch @change="handleChange" v-bind="fieldDesc.props" v-decorator="[field, Object.assign({ valuePropName: 'checked' }, fieldDesc.options)]" />
          <span v-if="fieldDesc.props && fieldDesc.props.ps" style="padding: 5px 7px" :style="fieldDesc.props.ps.style">{{ fieldDesc.props.ps.text }}</span>
        </template>
        <!-- 日期选择 -->
  ...
  methods: {
    ...
    handleChange(value, ...args) {
      ...
      if (typeof fieldDesc.onChange === "function") {
        fieldDesc.onChange(value, ...args);
      }
      this.$emit("change", value, ...args);
    },
    afterChange(value) {
